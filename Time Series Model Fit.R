...
ggplot(data)+
  geom_line(aes(x = 1:nrow(data), y = Close))
...
model_train = arima(train_data$Close, order = c(0,1,0))
y.fit = forecast(model_train, length(test_data$Close))[[4]]
...
#fitting the arima model with p,d,q
model_train = arima(train_data$Close, c(0,1,1))
y.fit = forecast(model_train, length(test_data$Close))[[4]]
...
#fitting the arima model with p,d,q
model_train = arima(train_data$Close, c(0,1,0))
y.fit = forecast(model_train, length(test_data$Close))[[4]]
...
#fitting the arima model with p,d,q
model_train = arima(train_data$Close, c(1,1,1))
y.fit = forecast(model_train, length(test_data$Close))[[4]]
